...
public class ClientService : IClientService
{
    ...
    {
        Client thisClient = await _clientStorage.GetClientAsync(x => x.TelegramId == client.TelegramId);
    ...
    {
        Executor thisExecutor = await _executorStorage.GetExecutorAsync(x => x.TelegramId == executor.TelegramId);

        if (executor is null || thisExecutor is null)
            return;
        Executor mappedExecutor = _mapper.Map<Executor>(executor);
